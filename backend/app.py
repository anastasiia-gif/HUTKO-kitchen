...
app.config['MAX_CONTENT_LENGTH'] = 12 * 1024 * 1024   # 12 MB uploads (product photos)
    ...
    try:
        ...
        for r in conn.execute("SELECT * FROM users ORDER BY created_at DESC").fetchall():
            ws2.append([r['id'], r['name'], r['email'], r['phone'], r['addr_street'],
                        r['addr_postcode'], r['addr_city'], r['addr_province'], r['created_at']])
        ...
        send_email(owner, f"HUTKO daily backup — {date_str}",
            f'<p style="font-family:Arial,sans-serif;color:#333;">Daily database backup for '
            f'<strong>HUTKO Kitchen</strong>, {date_str}.<br>Attached: <code>hutko_backup_{date_str}.xlsx</code></p>',
            attachments=[{'filename': f'hutko_backup_{date_str}.xlsx', 'content': xlsx_b64,
